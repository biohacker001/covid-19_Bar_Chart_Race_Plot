...
for i in range(len(df_csv['old_format_date'])):
   df_csv["Date"][i]=datetime.datetime.strptime(df_csv['old_format_date'][i], '%b %d %Y')
...
font = {'family' : 'cursive','weight' : 10, 'size'   : 15} # check https://matplotlib.org/3.2.1/tutorials/text/text_props.html for more details

def drawChart(date):
    # get the world total cases
    df2 = df_original[(df_original["Entity"]=="World") & (df_original["Date"]==date)].set_index("Entity", drop = False) 
